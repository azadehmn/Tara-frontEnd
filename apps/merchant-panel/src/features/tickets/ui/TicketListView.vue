    ...
    <TrCard>
      ...
      >
        ...
        <template #item-status="{ item }">
          <TrStatus :type="ticketStatusType(item.status)" :text="statusText(item.status)" />
        </template>
      </TrTable>
      <template #footer>
...
import { TrButton, TrCard, TrStatus, TrTable, type TrTableColumn } from '@tara/ui';
...
const columns = computed<TrTableColumn[]>(() => [
  { name: 'id', label: t('ticket.fields.id'), width: 'minmax(160px, 0.8fr)' },
